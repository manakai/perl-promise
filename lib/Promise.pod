...
If promises are used to write a standalone application, the condvar
returned by the C<to_cv> method of the last promise can be used to
wait for receiving, which effectively invokes the main loop of the
application:
